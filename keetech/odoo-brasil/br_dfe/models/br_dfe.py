...
class SpedDFeQueryWizard(models.TransientModel):
    ...
    def process_nsu(self, nsu):
        ...
        if schema == 'resNFe_v1.01.xsd':
            if hasattr(xml, 'CNPJ') == True:
                vals['cnpj'] = xml.CNPJ.text
            elif hasattr(xml, 'CNPJ') == False:
                vals['cnpj'] = xml.CPF.text
        ...
        # XML de NF-e de um Fornecedor
            ...
            if hasattr(xml.NFe.infNFe.emit, 'CNPJ') == True:
                vals['cnpj'] = xml.NFe.infNFe.emit.CNPJ.text
            elif hasattr(xml.NFe.infNFe.emit, 'CNPJ') == False:
                vals['cnpj'] = xml.NFe.infNFe.emit.CPF.text
