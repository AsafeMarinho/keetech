    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            order.update({
                ...
                'total_icms_st': sum(l.icms_st_valor for l in order.order_line),
                'total_desconto': sum(l.valor_desconto for l in order.order_line),
                'total_bruto': sum(l.valor_bruto for l in order.order_line),
                'total_des_icms': sum(l.icms_des_valor for l in order.order_line),
            })
...
class SaleOrderLine(models.Model):
    ...
    def _check_user_group_price_unit(self):
        ...
        if self.env.user.has_group('br_sale.group_unit_price_for_greater') == True:
            self.edit_price_unit = True
    ...
    def constrains_price_unit(self):
        ...
        if self.env.user.has_group('br_sale.group_unit_price_for_greater') == True:
            ...
            for record in self:
                price_default = self.env['account.tax']._fix_tax_included_price_company(
                    self._get_display_price(product), product.taxes_id, self.tax_id, self.company_id)
                if record.price_unit < price_default:
                    raise ValidationError("Produto %s esta com Vlr Unit. Menor que o valor padrão." % product.name)