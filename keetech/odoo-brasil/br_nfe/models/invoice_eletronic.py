...
try:
    ...
    from pytrustnfe.Servidores import localizar_url_nfce, localizar_qrcode
...
class InvoiceEletronic(models.Model):
    ...
    payment_ids = fields.One2many('nfe.pagamentos', 'invoice_eletronic_id', string=u'Dados de Pagamentos',
                                  readonly=True)

    # Compras
    nota_empenho = fields.Char(string="Nota de Empenho", size=22, readonly=True, states=STATE)
    pedido_compra = fields.Char(string="Pedido Compra", size=60, readonly=True, states=STATE)
    contrato_compra = fields.Char(string="Contrato Compra", size=60, readonly=True, states=STATE)
    sequencial_evento = fields.Integer(string=u"Sequêncial Evento", default=1, readonly=True, states=STATE)
    recibo_nfe = fields.Char(string=u"Recibo NFe", size=50, readonly=True, states=STATE)
    chave_nfe = fields.Char(string=u"Chave NFe", size=50, readonly=True, states=STATE)
    chave_nfe_danfe = fields.Char(string=u"Chave Formatado", compute="_format_danfe_key")
    protocolo_nfe = fields.Char(string=u"Protocolo", size=50, readonly=True, states=STATE,
        help=u"Protocolo de autorização da NFe")
    nfe_processada = fields.Binary(string=u"Xml da NFe", readonly=True)
    nfe_processada_name = fields.Char(string=u"Xml da NFe", size=100, readonly=True)
    valor_icms_uf_remet = fields.Monetary(string=u"ICMS Remetente", readonly=True, states=STATE,
        help=u'Valor total do ICMS Interestadual para a UF do Remetente')
    valor_icms_uf_dest = fields.Monetary(string=u"ICMS Destino", readonly=True, states=STATE,
        help=u'Valor total do ICMS Interestadual para a UF de destino')
    valor_icms_fcp_uf_dest = fields.Monetary(string=u"Total ICMS FCP", readonly=True, states=STATE,
    ...
    troco = fields.Monetary(string=u'Troco', digits=dp.get_precision('Account'), readonly=True)
 ...
 SEM VALOR FISCAL'
    ...
    def _prepare_eletronic_invoice_values(self):
        ...
        ide = {
            ...
            'tpEmis': int(self.tipo_emissao) if self.model == '55' else 9,
            #'dhCont': dt_format if self.model == '65' else None,
            #'xJust': 'Indisponibilidade de Internet' if self.model == '65' else None,
        ...
        #if self.model == '65':
        #    ide['dhCont'] = dt_format if self.model == '65' else None
        #    ide['xJust'] = 'Indisponibilidade de Internet' if self.model == '65' else None
        # Documentos Relacionados
        documentos = []
        ...
        if self.commercial_partner_id:
            ...
            if self.ambiente == 'homologacao':
                dest['xNome'] = u'NF-E EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL'
        ...
        if self.company_id.accountant_id:
            autorizados.append({'CNPJ': re.sub('[^0-9]', '', self.company_id.accountant_id.cnpj_cpf)})
        ...
        if self.model == '55':
            pag.append({
                ...
                'vPag': "%.02f" % self.fatura_liquido or '0.00',
                #'vTroco': "%.02f" % self.troco,
            })
        ...
        if self.model == '65':
            for p in self.payment_ids:
                pag.append({
                    'indPag': self.payment_term_id.indPag or '0',
                    'tPag': p.metodo_pagamento,
                    'vPag': "%.02f" % p.valor,
                })
            for p in pag:
                if p['tPag'] == '01':
                    p['vTroco'] = "%.02f" % self.troco
    ...
    @api.multi
    def reformulate_xml(self):
        '''Função para recriar o XML'''
        # TODO: Rever a funcao para recalcular os dados necessários de arquivos xml ja criados
