...
class WizardCartaCorrecaoEletronica(models.TransientModel):
    ...
    state = fields.Selection([('drat', u'Provisório'), ('error', u'Erro')], string=u"Situação")
    correcao = fields.Text(string=u"Correção", max_length=1000, required=True)
    eletronic_doc_id = fields.Many2one('invoice.eletronic', string=u"Documento Eletrônico")
    ...
    received_xml_name = fields.Char(string=u"Xml Recebimento", size=30, readonly=True)
    ...
    def send_letter(self):
        ...
        carta = {
            ...
            'eventos': [{
                ...
                'CNPJ': re.sub("[^0-9]", "", self.eletronic_doc_id.company_id.cnpj_cpf or ''),
                ...
                'idLote': self.id,
                'Id': "ID110110%s%02d" % (self.eletronic_doc_id.chave_nfe, numero_evento),
                #'modelo': self.eletronic_doc_id.model,
            }],
        }
        cert = self.eletronic_doc_id.company_id.with_context({'bin_size': False}).nfe_a1_file
        cert_pfx = base64.decodestring(cert)
        certificado = Certificado(cert_pfx, self.eletronic_doc_id.company_id.nfe_a1_password)
        ...
        if retorno.cStat == 128 and retorno.retEvento.infEvento.cStat in (135, 136):
            ...
            self.write({
                'state': 'error',
                'message': mensagem,
                'sent_xml': base64.b64encode(resposta['sent_xml'].encode('utf-8')),
                'sent_xml_name': 'cce-envio.xml',
                'received_xml': base64.b64encode(resposta['received_xml'].encode('utf-8')),
