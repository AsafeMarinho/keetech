...
class AccountInvoice(models.Model):
    ...
    def action_invoice_cancel_paid(self):
        receivable_lines = self.receivable_move_line_ids
        receivable_boletos = self.env['payment.order.line']
        for move_line in receivable_lines:
            boleto = receivable_boletos.search([('move_line_id', '=', move_line.id),
                                                      ('payment_order_id.state', '=', 'draft')])
            if boleto:
    ...
    def send_email_boleto_queue(self):
        ...
        for item in self:
            ...
            if boleto:
                name = "boleto-%s-%s.pdf" % (item.number, item.partner_id.commercial_partner_id.name)
