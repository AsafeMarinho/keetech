...
class danfce(object):
    ...
    def __init__(self, list_xml, logo=None):
        ...
        self.items = count_items(self.list_xml)
        self.page_height = 12 * cm + (self.items * (0.7 * cm))
        self.page_width = 7.2 * cm
        self.max_height = self.page_height - 1 * cm
        self.min_height = 1
        self.min_width = 5
        self.max_width = 200
        self.current_height = self.page_height - 1 * cm

        self.oPDF_IO = BytesIO()
        self.canvas = canvas.Canvas(self.oPDF_IO, pagesize=(self.page_width, self.page_height))
    ...
    def ide_emit(self, oXML=None):
        ...
        cEnd += tagtext(oNode=elem_emit, cTag='xLgr') + ', ' + tagtext(oNode=elem_emit, cTag='nro') + ' - '
        cEnd += tagtext(oNode=elem_emit, cTag='xBairro') + '<br />' + tagtext(oNode=elem_emit, cTag='xMun') + ' - '
        cEnd += tagtext(oNode=elem_emit, cTag='UF') + ' - ' + tagtext(oNode=elem_emit, cTag='CEP') + '<br />'
    ...
    def totais(self, oXML=None):
        ...
        valor_a_pagar = format_number(tagtext(oNode=el_total, cTag='vNF'),
                                      precision=2)
        pag = oXML.find(".//{http://www.portalfiscal.inf.br/nfe}pag")
        troco = format_number(tagtext(oNode=pag, cTag='vTroco'), precision=2)
        all_pag = pag.findall(".//{http://www.portalfiscal.inf.br/nfe}detPag")
    ...
    def draw_totals_table(self, values):
        ...
        for item in values['formas_de_pagamento']:
            ...
            rowHeights.append(7)
        data.append(['TROCO', values['troco']])
        rowHeights.append(7)
...
%s<br />%s' % (url_chave, access_key)
    ...
    def nfce_generate(self):
        for oXML in self.list_xml:
            ...
            if el_det is not None:
                ...
                for nId, item in enumerate(el_det):
                    #nIt += 1
                    #self.page_height += self.nIt * (0.7 * cm)
                    el_prod = item.find(".//{http://www.portalfiscal.inf.br/nfe}prod")
            ...
            #self.current_height = self.page_height
            #self.max_height = self.page_height
            self.drawLine()
        ...
        #if self.nIt == 1:
        #    p_height = self.page_height
        #else:
        #    p_height = self.page_height
            #p_height = self.page_height + (((self.nIt - 1) * 0.3) * cm)
        #self.canvas.setPageSize((self.page_width, p_height))
        self.newpage()
